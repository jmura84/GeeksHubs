...
class TestSkip(unittest.TestCase):
    ...
    def requires_numpy18(self):
        ...
        return unittest.skipUnless(NUMPY_VER) >= StrictVersion('1.8'), 'Requires Numpy >= 1.8'

    def requires_numpy_previous_than(version):
        if not HAS_NUMPY:
            return unittest.skip('Requires Numpy')
        return unittest.skipUnless(NUMPY_VER) < StrictVersion(version), 'Requires Numpy < 1.8'
